...
// reasoning: WEEK3_ARCHITECTURE.md §6A scopes that registry to the §7-§23 learner-intelligence
...
// Retained verbatim from the Revision 1 design WEEK3_ARCHITECTURE.md §6 explicitly keeps ("that
...
/**
 * Canonical concept identity (WEEK3_ARCHITECTURE.md §6's `concept_key` -- the task's "slug" is
...
/**
 * Canonical subject/domain identity (WEEK3_ARCHITECTURE.md §9.1's `subject` column, shared by
...
// orchestrator" split WEEK3_ARCHITECTURE.md §3/§17 uses throughout, and it's what makes the DAG
 ...
 * findPrereqCycle (WEEK3_ARCHITECTURE.md §36: ADAPT DIRECTLY) including its self-loop fast path.
 ...
 * tie-break convention WEEK3_ARCHITECTURE.md §17.2 already uses for concept/action selection.
 */
 ...
 * (WEEK3_ARCHITECTURE.md §6/Revision 1: "insert-on-conflict by concept_key, append the raw input
 ...
 * mastery of each prerequisite (WEEK3_ARCHITECTURE.md §6's `isReadyFor`), which requires
