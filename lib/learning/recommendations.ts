// Revision recommendations (WEEK3_ARCHITECTURE.md §23, Phase 11). Module path matches the
    ...
    ranked.map(async (item) => {
      ...
      const blockerEntries = await Promise.all(
        readiness.blockers.map(async (blocker) => {
          ...
          return { conceptId: blocker.conceptId, conceptKey: blocker.conceptKey, displayName: blocker.displayName, subject: item.subject, priority, reasonCodes, summary: formatRevisionSummary(blocker.displayName, blockerSignal.pMastery, reasonCodes) };
        }),
      );
  ...
  const transferCandidates = await Promise.all(
    signals
      ...
      .map(async (s) => {
        ...
        return { conceptId: s.conceptId, conceptKey: s.conceptKey, displayName: s.displayName, subject: s.subject, priority: 1 - (s.pMastery ?? 1), reasonCodes, summary: formatRevisionSummary(s.displayName, s.pMastery, reasonCodes) } as RevisionRecommendation;
      }),
  );
