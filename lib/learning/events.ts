...
// bounds on the event ledger itself (WEEK3_ARCHITECTURE.md §6A scopes that registry to "every
...
export async function recordLearningEvent(input: RecordLearningEventInput, dependencies: EventDependencies = {}): Promise<LearningEvent> {
  ...
  if (error) {
    if (error.code === UNIQUE_VIOLATION && idempotencyKey) {
      ...
      // WEEK3_ARCHITECTURE.md §20 locks this in as "one column, one index, not a subsystem."
