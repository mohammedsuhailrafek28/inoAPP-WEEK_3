// Centralized, versioned tunable-constants registry (WEEK3_ARCHITECTURE.md §6A, Decision 1 — LOCKED).
//
...
// parameter, a client-writable field, or an admin UI (WEEK3_ARCHITECTURE.md §38 item 21).
//
...
// silently editing a value in place (WEEK3_ARCHITECTURE.md §6A/§20). Every persisted mastery
...
export const MODEL_PARAMETERS = Object.freeze({
  // BKT (WEEK3_ARCHITECTURE.md §7.4) -- global fallbacks. P(L0)/P(T) are per-concept-overridable
  ...
  BKT_DEFAULT_P_L0: defineConstant(0.2, "OUR_CHOICE", "Conservative 'assume not yet known' prior, within the commonly-cited 0.1-0.3 range for Corbett & Anderson-style BKT deployments; not fitted to real data (WEEK3_ARCHITECTURE.md §7.4)."),
  BKT_DEFAULT_P_T: defineConstant(0.3, "OUR_CHOICE", "Within the commonly-cited BKT learning-rate range (WEEK3_ARCHITECTURE.md §7.4)."),
  BKT_DEFAULT_P_S_MCQ: defineConstant(0.1, "OUR_CHOICE", "No Tutor MCP equivalent -- a per-item-type split we introduce (WEEK3_ARCHITECTURE.md §7.4)."),
  BKT_DEFAULT_P_S_SHORT_ANSWER: defineConstant(0.15, "OUR_CHOICE", "Higher than MCQ to account for LLM-graded rubric noise (WEEK3_ARCHITECTURE.md §7.4)."),
  BKT_DEFAULT_P_G_MCQ: defineConstant(0.22, "OUR_CHOICE", "Approximates a 1-in-~4.5 chance guess on a typical MCQ (WEEK3_ARCHITECTURE.md §7.4)."),
  BKT_DEFAULT_P_G_SHORT_ANSWER: defineConstant(0.05, "OUR_CHOICE", "Near-zero blind-guess chance for free text (WEEK3_ARCHITECTURE.md §7.4)."),
  BKT_FORGET_WITHIN_SESSION: defineConstant(
    ...
    "Tutor MCP exposes a live P(Forget) field but never mutates it at any call site the audit " +
      "found; we fix it as a small constant rather than carry unused flexibility (WEEK3_ARCHITECTURE.md §7.2). " +
  ...
  // PFA (WEEK3_ARCHITECTURE.md §8) -- ported verbatim, symmetric, no intercept.
  PFA_BETA_SUCCESS: defineConstant(0.11, "TUTOR_MCP_CHOICE", "Adopted verbatim from the audited source (WEEK3_ARCHITECTURE.md §8)."),
  PFA_BETA_FAILURE: defineConstant(-0.11, "TUTOR_MCP_CHOICE", "Adopted verbatim from the audited source (WEEK3_ARCHITECTURE.md §8)."),
  // IRT (WEEK3_ARCHITECTURE.md §9) -- 1PL/Rasch (no discrimination parameter -- see the "IRT model
  // re-review" note at §6A/§38: nothing in this design ever consumes one).
  IRT_THETA_BOUNDS: defineConstant(
    { min: -4, max: 4 },
    "TUTOR_MCP_CHOICE",
    "The domain ability θ is clamped to, adopted verbatim (WEEK3_ARCHITECTURE.md §9.3). Categorized " +
  ...
  IRT_BASE_PRIOR_PRECISION: defineConstant(
    ...
    "The regularization anchor for the Newton-step update -- grows with observation_count so one " +
      "response can never saturate θ (WEEK3_ARCHITECTURE.md §9.3).",
  ),
  IRT_INFO_PER_OBSERVATION: defineConstant(
    ...
    "The maximum Fisher information of a discrimination-1 2PL item is a mathematical property of " +
      "the model, not a tuned choice (WEEK3_ARCHITECTURE.md §9.3).",
  ),
  IRT_MAX_NEWTON_STEP: defineConstant(1.0, "TUTOR_MCP_CHOICE", "The magnitude one Newton step is clamped to before being applied to θ (WEEK3_ARCHITECTURE.md §9.3)."),
  IRT_ITEM_DIFFICULTY_B: defineConstant(
    ...
    "A fixed, auditable mapping from the product difficulty label to IRT's continuous b -- no FSRS-" +
      ...
      "per-question label, not derived from a review-scheduling difficulty (WEEK3_ARCHITECTURE.md §9.2).",
  ),
  // FSRS (WEEK3_ARCHITECTURE.md §10.2) -- ported verbatim, the Open-Spaced-Repetition project's own
  ...
  FSRS_WEIGHTS: defineConstant(
    ...
    "PUBLISHED_ALGORITHM",
    "The Open-Spaced-Repetition project's own published default parameterization, ported verbatim (WEEK3_ARCHITECTURE.md §10.2/§6A).",
  ),
  FSRS_FACTOR: defineConstant(19 / 81, "PUBLISHED_ALGORITHM", "The retrievability/next-interval formulas' scale constant, ported verbatim (WEEK3_ARCHITECTURE.md §10.2)."),
  FSRS_DECAY: defineConstant(-0.5, "PUBLISHED_ALGORITHM", "The retrievability formula's exponent, ported verbatim (WEEK3_ARCHITECTURE.md §10.2)."),
  FSRS_DESIRED_RETENTION: defineConstant(0.9, "PUBLISHED_ALGORITHM", "The target recall probability the next-interval formula solves for -- a common FSRS operational convention, adopted by both Tutor MCP and us (WEEK3_ARCHITECTURE.md §10.2)."),
});
...
export const PRODUCT_POLICY_THRESHOLDS = Object.freeze({
  STALE_SESSION_MINUTES: defineConstant(
    ...
    "Decision 2 (locked): a study session may include long PDF-reading/problem-solving gaps away " +
      ...
      "actually stopped. See WEEK3_ARCHITECTURE.md §31.",
  ),
  MAX_PREREQUISITE_TRAVERSAL_DEPTH: defineConstant(
    ...
    "Phase 2, Step 13: a safety bound on prerequisite-graph traversal (closures, topological " +
      ...
      "to be a small fraction of this. Cycle prevention (WEEK3_ARCHITECTURE.md §6) already keeps " +
  ...
  MASTERY_ACHIEVED_THRESHOLD: defineConstant(
    0.85,
    "TUTOR_MCP_CHOICE",
    "Their unified-profile value, adopted (WEEK3_ARCHITECTURE.md §7.3). The 'hard' BKT mastery " +
  ...
  MIN_EVIDENCE_FOR_ADAPTIVE: defineConstant(
    ...
    "Below this many scored opportunities on a concept (BKT) or observations on a subject (IRT), " +
      ...
      "posterior/theta as a confident verdict (WEEK3_ARCHITECTURE.md §7.5/§9.4).",
  ),
  // Adaptive difficulty hysteresis (WEEK3_ARCHITECTURE.md §16) -- the exact Revision 3 thresholds,
  ...
  // IRT sanity modifier band (WEEK3_ARCHITECTURE.md §9.4/§16 step 3) -- distinct from the ZPD band
  ...
  // The ZPD band (WEEK3_ARCHITECTURE.md §9.4) -- reused directly from the audited source. Not yet
  ...
  IRT_ZPD_LOWER: defineConstant(0.55, "TUTOR_MCP_CHOICE", "WEEK3_ARCHITECTURE.md §9.4 -- reused directly from the audited source's ZPD band."),
  IRT_ZPD_UPPER: defineConstant(0.80, "TUTOR_MCP_CHOICE", "WEEK3_ARCHITECTURE.md §9.4 -- reused directly from the audited source's ZPD band."),
  ...
  // Phase FSM (WEEK3_ARCHITECTURE.md §17.1, Phase 9) -- named distinctly from
  ...
  // Prerequisite readiness (WEEK3_ARCHITECTURE.md §6) -- deliberately distinct from
  ...
  // Misconceptions (WEEK3_ARCHITECTURE.md §11).
  ...
  // Transfer (WEEK3_ARCHITECTURE.md §12). Two distinct thresholds are both literally present in
  ...
  // Calibration (WEEK3_ARCHITECTURE.md §13).
  ...
  // Scaffolding (WEEK3_ARCHITECTURE.md §15) -- the exact Revision 3 values ("bounds corrected in
  ...
  // Open Learner Model presentation (WEEK3_ARCHITECTURE.md §30.2, Phase 10). Named distinctly from
...
export const SAFETY_CLAMPS = Object.freeze({
  ...
  BKT_BAYES_DENOMINATOR_FLOOR: defineConstant(1e-9, "TUTOR_MCP_CHOICE", "Anti-division-by-zero guard on the Bayesian update's denominator (WEEK3_ARCHITECTURE.md §7.2)."),
  IRT_NEWTON_STEP_CLAMP: defineConstant(
    { min: -1, max: 1 },
    "TUTOR_MCP_CHOICE",
    "The operative clamp applied to the raw Newton step before it moves θ (WEEK3_ARCHITECTURE.md " +
  ...
  FSRS_STABILITY_DIFFICULTY_FLOOR: defineConstant(
    1e-9,
    "PUBLISHED_ALGORITHM",
    "Anti-Pow(0,-k)-Infinity guard inherent to the retrievability/stability formulas, not a tuned choice -- floors stability before it is ever raised to a negative power (WEEK3_ARCHITECTURE.md §10.2/§6A).",
  ),
  FSRS_DIFFICULTY_BOUNDS: defineConstant({ min: 1, max: 10 }, "PUBLISHED_ALGORITHM", "The clamp initialDifficulty() applies, ported verbatim (WEEK3_ARCHITECTURE.md §10.2)."),
  FSRS_MIN_INTERVAL_DAYS: defineConstant(1, "PUBLISHED_ALGORITHM", "The next-interval formula's own max(1, ...) floor, ported verbatim -- a review is never scheduled zero or negative days out (WEEK3_ARCHITECTURE.md §10.2)."),
  CALIBRATION_RANGE: defineConstant({ min: 0, max: 1 }, "OUR_CHOICE", "Mathematical necessity, not a policy choice (WEEK3_ARCHITECTURE.md §6A) -- predicted/actual are both derived from bounded scales (a 1-5 Likert rating, a correctness fraction) and enforced via a DB CHECK, not tunable."),
  // Quiz generation validation gates (WEEK3_ARCHITECTURE.md §19, Phase 9) -- guards against a
