...
test("the Progress/Practice/Profile panels only ever call existing, already-tested API routes -- no new endpoint was invented for frontend convenience", () => {
  const allowedPrefixes = ["/api/rag", "/api/chat", "/api/documents", "/api/profile", "/api/learning/progress", "/api/learning/concepts", "/api/quiz/generate", "/api/quiz/"];
  for (const file of clientSourceFiles()) {
    const source = readFileSync(file, "utf8");
