...
-- (WEEK3_ARCHITECTURE.md §25, §27, §31).
...
-- (WEEK3_ARCHITECTURE.md §27); also what makes "supersede the prior session" in
...
create table if not exists public.learning_events (
  ...
  event_type text not null check (event_type in (
    -- Full catalog per WEEK3_ARCHITECTURE.md §25, so later phases never need to ALTER this
...
-- deliberately the entire idempotency mechanism (WEEK3_ARCHITECTURE.md §20): one column, one
...
-- Practical immutability (Step 8 / WEEK3_ARCHITECTURE.md §20, §25): no application code path ever
